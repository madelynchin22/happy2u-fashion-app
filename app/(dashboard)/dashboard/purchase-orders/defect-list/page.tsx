...
export default function DefectListPage() {
  ...
  const totalDefects = items.reduce((s, i) => s + (i.defectQty ?? 0), 0);
  const totalMissing = items.reduce((s, i) => s + (i.missingQty ?? 0), 0);
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Defects and missing pairs reported upon stock arrival at each outlet</p>
        </div>
        {!loading && items.length > 0 && (
          <div className="flex items-center gap-6">
            ...
            {totalMissing > 0 && (
              <div className="text-right">
                <p className="text-2xl font-bold text-amber-500">{totalMissing}</p>
                <p className="text-xs text-gray-400">total missing pairs</p>
              </div>
            )}
          </div>
        )}
      ...
      {!loading && items.length === 0 && (
        <div className="border border-dashed border-gray-200 rounded-xl p-16 text-center">
          ...
          <p className="text-sm text-gray-400">No defects or missing pairs reported yet</p>
          <p className="text-xs text-gray-300 mt-1">Cases will appear here when goods receipt is recorded with defects or missing pairs</p>
        </div>
      )}

      {!loading && Object.values(byPO).map(({ po, rows }) => {
        const poTotal = rows.reduce((s, r) => s + (r.defectQty ?? 0), 0);
        const poMissing = rows.reduce((s, r) => s + (r.missingQty ?? 0), 0);
        return (
          <div key={po.id} className="border border-gray-100 rounded-xl overflow-hidden">
            ...
            <div className="bg-gray-50 px-5 py-3 flex items-center justify-between border-b border-gray-100">
              ...
              <div className="flex items-center gap-3">
                {poTotal > 0 && <span className="text-xs font-semibold text-red-500">{poTotal} defects</span>}
                {poMissing > 0 && <span className="text-xs font-semibold text-amber-500">{poMissing} missing</span>}
                <Link
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-xs text-gray-400 uppercase tracking-wide">
                  ...
                  <th className="text-center px-3 py-2 text-amber-500">Missing</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {rows.map(row => (
                  <tr key={row.id} className="hover:bg-red-50/30 transition-colors">
                    ...
                    <td className="px-3 py-3 text-center">
                      {(row.missingQty ?? 0) > 0 ? (
                        <span className="inline-flex items-center justify-center w-8 h-6 bg-amber-100 text-amber-600 rounded font-bold text-xs">
                          {row.missingQty}
                        </span>
                      ) : <span className="text-gray-300">—</span>}
                    </td>
