...
import { RefreshCw, Plus, ExternalLink, ShoppingBag, Search, X, PackageSearch } from "lucide-react";
...
export default function CompetitorsPage() {
  ...
  const tabCounts = { new: 0, restock: 0, all: 0 };
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <a href={c.url} target="_blank" rel="noopener noreferrer"
                  ...
                  {c.url.replace("https://","").replace("www.","").replace(/\/$/,"")}
                    ...
                    <a key={p.id} href={p.productUrl} target="_blank" rel="noopener noreferrer"
                      ...
                      <div className="aspect-square overflow-hidden bg-gray-100 relative">
                        ...
                        <div className="absolute top-2 left-2 flex flex-col gap-1">
                          {p.isNew && (
                            <span className="bg-brand-600 text-white text-[10px] px-1.5 py-0.5 rounded font-bold">NEW</span>
                          )}
                          {p.wasRestocked && (
                            <span className="bg-amber-500 text-white text-[10px] px-1.5 py-0.5 rounded font-bold">RESTOCK</span>
                          )}
                          {!p.isAvailable && (
                            <span className="bg-gray-500 text-white text-[10px] px-1.5 py-0.5 rounded">OOS</span>
                          )}
                        </div>
                      </div>
                      <div className="p-2.5">
                        ...
                        {colors.length > 0 && (
                          <p className="text-[10px] text-gray-400 mt-0.5 truncate">{colors.join(", ")}</p>
                        )}
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="card w-full max-w-md p-6">
            ...
            {pageError && (
              <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">{pageError}</div>
            )}
