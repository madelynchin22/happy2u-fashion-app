...
export function PurchaseOrderPDF({ po }: { po: any }) {
  ...
  // Group items by model key using a Map — order-independent, works regardless of item order in DB
  const groupMap = new Map<string, { key: string; supplierSku: string; items: any[] }>();
  const groupOrder: string[] = [];
  for (const item of items) {
    const key = modelKey(item);
    const displaySku = item.supplierSku || item.mainSku || key || item.h2uSku || "";
    if (!groupMap.has(key)) {
      groupMap.set(key, { key, supplierSku: displaySku, items: [] });
      groupOrder.push(key);
    }
